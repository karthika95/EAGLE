...
def warmup(model):
    ...
    if args.model_type == "tulu":
        conv = get_conversation_template("tulu")
        conv.append_message(conv.roles[0], "नमस्कार")
        conv.append_message(conv.roles[1], None)
...
def bot(history, temperature, top_p, use_EaInfer, highlight_EaInfer,session_state,):
    ...
    generation_id = 0
    currpath = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    graphdir = os.path.join(currpath, "Graphs")#Directory to store the graphs and json
    if not os.path.exists(graphdir):
        os.mkdir(graphdir)

    Path(graphdir,"Image").mkdir(exist_ok=True)
    Path(graphdir,"Json").mkdir(exist_ok=True)

    image_path=os.path.join(graphdir,"Image")
    json_path=os.path.join(graphdir,"Json")

    clear_folder(image_path)
    clear_folder(json_path)   
    
    if not history:
        return history, "0.00 tokens/s", "0.00", session_state
    pure_history = session_state.get("pure_history", [])
    assert args.model_type in ["llama-2-chat", "vicuna", "llama-3-instruct", "tulu"]
    conv = get_conversation_template(args.model_type)
    ...
    if use_EaInfer:
        ...
        for output_ids in model.ea_generate(input_ids, temperature=temperature, top_p=top_p,
            ...
            # Output ids is system prompt + user prompt + generated tokens 
            total_ids+=1
            ...
            # Text consists of first token generated by target LLM, and multiple accepted drafts from EAGLE-3, upto but not including next white token, for each iteration.
            naive_text.append(model.tokenizer.decode(output_ids[0, cu_len], skip_special_tokens=True,
                                                     spaces_between_special_tokens=False,
                                                     clean_up_tokenization_spaces=True, ))
            # Naive text is list of tokens by target LLM (white text)
            # print("Draft token:", naive_text[-1])#updated_njk
            
            print("EAGLE-3 tokens:", model.tokenizer.convert_ids_to_tokens(output_ids[0, cu_len+1:]))

            cu_len = output_ids.shape[1]
            highlight_EaInfer=True
...
def launch_web():
    ...
    time.sleep(2)
    webbrowser.open("http://192.168.10.234:5000")
    webbrowser.open_new_tab(url="http://192.168.10.234:5005")
...
parser.add_argument("--model-type", type=str, default="vicuna",choices=["llama-2-chat","vicuna","mixtral","llama-3-instruct", "tulu"])
parser.add_argument(
    "--total-token",
    type=int,
    default=60,
...
# Pranav: Disabling warmup for unnecesssary tree generation
# warmup(model)
...
with gr.Blocks(css=custom_css) as demo:
    ...
    gr.Markdown('''## BharatGen Multi-Token Prediction for Indian Languages''')
    with gr.Row():
    ...
    with gr.Row():
        with gr.Column():
            use_EaInfer = gr.Checkbox(label="Use Draft Model", value=True)
            highlight_EaInfer = gr.Checkbox(label="Highlight the tokens generated by Draft Model", value=True)
    ...
    note=gr.Markdown(show_label=False,value='''The Compression Ratio is defined as the number of generated tokens divided by the number of forward passes in the original LLM. If "Highlight the tokens generated by draft model" is checked, the tokens correctly guessed by draft model 
    ...
    with gr.Row():
        note=gr.Markdown(show_label=False,value='''<div align="center">  
        <b>Acknowledgements</b><br>
        Karthika N. J.*, Pranav Shinde*, Jayaraj Poroor<sup>†</sup>, Ganesh Ramakrishnan*, Arnab Bhattacharya<sup>‡</sup><br>  
        <small>*IIT Bombay&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<sup>†</sup>Zoho Labs, Kottarakkara&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<sup>‡</sup>IIT Kanpur</small>  
        </div>
        ''')
